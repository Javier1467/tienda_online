...
const initializeDatabase = async () => {
  try {
    ...
    // Adaptar instalaciones existentes sin borrar sus pedidos.
    const columnasPedido = [
      ["subtotal", "DECIMAL(10, 2) NOT NULL DEFAULT 0"],
      ["total", "DECIMAL(10, 2) NOT NULL DEFAULT 0"],
      ["estado", "VARCHAR(30) NOT NULL DEFAULT 'Pendiente'"],
    ]
    for (const [nombre, definicion] of columnasPedido) {
      const [columnas] = await connection.query(
        "SELECT COUNT(*) AS cantidad FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'pedido' AND COLUMN_NAME = ?",
        [nombre],
      )
      if (columnas[0].cantidad === 0) {
        await connection.query(`ALTER TABLE pedido ADD COLUMN ${nombre} ${definicion}`)
      }
    }
